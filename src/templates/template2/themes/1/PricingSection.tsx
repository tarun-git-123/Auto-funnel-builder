        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-book-blue-900">
              Our <span className="bg-clip-text text-transparent" style={{
                backgroundImage: `linear-gradient(to right, ${config.primaryColor}, ${config.secondaryColor})`,
              }}>Pricing</span> Packages
            </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {pricingTiers.map((tier, index) => (
            <motion.div
              ...
              className={`card-hover rounded-xl overflow-hidden relative ${tier.popular ? "ring-2 ring-book-yellow-500 ring-offset-2 ring-offset-book-blue-50" : ""
                }`}
            >
              {tier.popular && (
              ...
              <div className="p-6">
                ...
                {/* CTA */}
                <Link
                  href={`/checkout?package=${encodeURIComponent(tier.name)}&price=${tier.price.toFixed(2)}`}
                  className={`block w-full py-3 px-4 text-center rounded-lg font-medium transition-colors ${tier.popular
                    ? "text-white"
